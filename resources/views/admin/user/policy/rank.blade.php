...
<div class="body-wrapper">
    <div class="container-fluid">
        <ul class="nav nav-tabs mt-3" id="tableTabs" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <a href="{{ route('admin.user.policy', ['mode' => 'rank']) }}" class="nav-link active">
                    승급보너스
                </a>
            </li>
        </ul>
        <div class="card">
            <div class="card-body">
                <div class="mb-3 d-flex justify-content-between">
                    <h5 class="card-title">승급보너스 정책</h5>
                </div>
                <hr>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <form method="POST" action="{{ route('admin.user.policy.store') }}" id="ajaxForm" data-confirm-message="정책을 추가하시겠습니까?" >
                    @csrf
                                            ...
                                            @if(is_array($oldConditions))
                                                <ul class="list-unstyled mb-0">
                                                    @foreach($oldConditions as $item)
                                                        <li>최소 레벨: {{ $item['min_level'] ?? '?' }},  인원 수: {{ $item['referral_count'] ?? '?' }}</li>
                                                    @endforeach
                                                </ul>
                                            @else
                                                {{ $val->old_value }}
